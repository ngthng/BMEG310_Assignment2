...
```{r}
train_datasubset = ovarian.dataset.train[, c(2:ncol(ovarian.dataset))]

for (i in 1:nrow(train_datasubset)){
  train_datasubset$diagnosis[i] = ifelse(train_datasubset$diagnosis[i] == "M", 1, 0)
  print(i)
}
...
```{r}
glm.fit = glm(diagnosis ~ perimeter + area + smoothness + symmertry + concavity, data = train_datasubset, type = "Response" )
```
